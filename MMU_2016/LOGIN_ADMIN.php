...
if(isset($_COOKIE['ID_your_site'])){ //if there is, it logs you in and directes you to the members page
 ...
 if (isset($_POST['submit'])) {
	...
	// makes sure they filled it in
 	...
 	$check = mysqli_query($conect, "SELECT * FROM admin WHERE id_number = '".$_POST['username']."'")or die(mysql_error());

 //Gives error if user dosen't exist
 $check2 = mysqli_num_rows($check);
 if ($check2 == 0){
	die('That user does not exist in our database.<br /><br />If you think this is wrong <a href="LOGIN_ADMIN.php">try again</a>.');
}
...
while($info = mysqli_fetch_array( $check )){
	...
	//gives error if the password is wrong
 	if ($_POST['pass'] != $info['password']){
 		die('Incorrect password, please <a href="LOGIN_ADMIN.php">try again</a>.');
 	}
		...
		header("Location: Generate_Passcode.php"); 
	}
}
}
else{
